...
export default function Process({ deliveryProcess }) {
  ...
  return (
    <section id="process" className="relative z-10 border-t border-neutral-900/80 py-28 px-6 max-w-7xl mx-auto space-y-24" aria-labelledby="process-title">
      ...
      <div className="relative max-w-4xl mx-auto before:absolute before:inset-0 before:left-4 md:before:left-1/2 before:w-[1px] before:bg-gradient-to-b before:from-neutral-900 via-neutral-800 to-neutral-900/20 before:pointer-events-none">
        {refinedSteps.map((proc, idx) => (
          <div key={idx} className={`relative flex flex-col md:flex-row items-start mb-12 last:mb-0 ${idx % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
            ...
            <div className="absolute left-4 md:left-1/2 -translate-x-1/2 w-8 h-8 rounded-full bg-neutral-950 border border-neutral-800 flex items-center justify-center z-10 shadow-[0_0_15px_rgba(0,0,0,0.9)] group">
            ...
            <div className="w-full md:w-1/2 pl-14 md:pl-0 ${idx % 2 === 0 ? 'md:pr-14 text-left md:text-right' : 'md:pl-14 text-left'}">
              <div className="bg-gradient-to-b from-neutral-950/70 to-neutral-950/40 backdrop-blur-md border border-neutral-900 p-6 rounded-2xl hover:border-neutral-800/80 transition-all duration-300 group shadow-sm w-full relative overflow-hidden">
                
                {/* VISUAL INDICATOR ACCENT */}
                <div className={`absolute top-0 w-12 h-[1px] bg-gradient-to-r from-[#06B6D4]/40 to-transparent ${idx % 2 === 0 ? 'right-6' : 'left-6'}`} />
                
                <h3 className="text-sm font-bold text-white mb-2 font-mono uppercase tracking-wider block">
                  {proc.label}
                </h3>
                
                <p className="text-xs sm:text-sm text-neutral-400 leading-relaxed font-normal font-sans">
                  {proc.desc}
                </p>
